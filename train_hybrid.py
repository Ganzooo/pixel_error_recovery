    ...
    for _dl in dataloader:
        ...
        for step, (data) in pbar:
            ...
            if use_mask_loss:
                ...
                _image_patch, _gt_patch, _org_img, _mask = _image_patch.to(device), _gt_patch.to(device), _org_img.to(
                    device), _mask.to(device)
            else:
...
def valid_one_epoch(cfg, model, dataloader, criterion, device, epoch, stat_dict, run_log_wandb):
    ...
    for _dl in dataloader:
        ...
        for _image_patch, _gt_patch, _idx, _fname, _imageOrg_patch in pbar:
            ...
            if cfg.train_config.pixel_recovery:
                ...
                for b in range(_pred.shape[0]):
                    ...
                    if cfg.train_config.save_img_rec:
                        ...
                        save_img(os.path.join(dirPath, str(epoch) + '_rec', fname), _rec.numpy().astype(np.uint8),
                                 color_domain='rgb')
            ...
            pbar.set_postfix(epoch=f'{epoch}', acc=f'{acc_rec:0.4f}', val_loss=f'{val_loss_meter.avg:0.2f}',
                             psnr=f'{psnr:0.2f}')
