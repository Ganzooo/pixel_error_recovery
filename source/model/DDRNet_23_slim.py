...
class DualResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=19, in_channel=1, planes=64, spp_planes=128, head_planes=128, augment=False):
    ...
    def forward(self, x):
        ...
        out = F.interpolate(x_, size=[height_in, width_in], mode='bilinear')
        if self.augment: 
...
def DualResNet_imagenet(pretrained=False, n_classes=1):
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=n_classes, in_channel=1, planes=32, spp_planes=128, head_planes=64, augment=False)
    if pretrained:
