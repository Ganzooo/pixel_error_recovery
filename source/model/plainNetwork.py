...
class plainHYBRID(nn.Module):
    def __init__(self, module_nums=4, channel_nums=32, num_class=2, act_type='relu', colors=3, use_bn_det=True,
                 use_bn_rec=True, rec_mode=True):
        ...
        backbonePD = []
        backbonePD += [Conv3X3(inp_planes=self.colors, out_planes=self.channel_nums, act_type=self.act_type,
                               use_bn=self.use_bn_det)]
        for _ in range(self.module_nums):
            backbonePD += [Conv3X3(inp_planes=self.channel_nums, out_planes=self.channel_nums, act_type=self.act_type,
                                   use_bn=self.use_bn_det)]
        backbonePD += [Conv3X3(inp_planes=self.channel_nums, out_planes=self.channel_nums // 2, act_type='linear',
                               use_bn=self.use_bn_det)]
        ...
        backbonePR = []
        backbonePR += [Conv3X3(inp_planes=self.colors, out_planes=self.channel_nums, act_type=self.act_type,
                               use_bn=self.use_bn_rec)]
        for _ in range(self.module_nums):
            backbonePR += [Conv3X3(inp_planes=self.channel_nums, out_planes=self.channel_nums, act_type=self.act_type,
                                   use_bn=self.use_bn_rec)]
        backbonePR += [Conv3X3(inp_planes=self.channel_nums, out_planes=self.channel_nums, act_type='linear',
                               use_bn=self.use_bn_rec)]
        self.backbonePR = nn.Sequential(*backbonePR)

        self.trasitionPR = nn.Sequential(
            Conv3X3(inp_planes=self.channel_nums, out_planes=self.channel_nums // 2, act_type=self.act_type,
                    use_bn=self.use_bn_rec),
            Conv1X1(inp_planes=self.channel_nums // 2, out_planes=self.colors, act_type=self.act_type,
                    use_bn=self.use_bn_rec)
        )
...
class shuffleHYBRID(nn.Module):
    def __init__(self, module_nums=4, channel_nums=32, num_class=2, act_type='relu', colors=3, use_bn_det=True,
                 use_bn_rec=True, rec_mode=True, downscale_size=2):
        super(plainHYBRID, self).__init__()
        self.module_nums = module_nums
        self.channel_nums = channel_nums
        self.colors = colors
        ...
        self.downscale = nn.PixelUnshuffle(self.downscale_size)
        self.upscale = nn.PixelShuffle(self.downscale_size)

        backbonePD = []
        backbonePD += [
            Conv3X3(inp_planes=self.colors * downscale_size ** 2, out_planes=self.channel_nums, act_type=self.act_type,
                    use_bn=self.use_bn_det)]
        for _ in range(self.module_nums):
            backbonePD += [Conv3X3(inp_planes=self.channel_nums, out_planes=self.channel_nums, act_type=self.act_type,
                                   use_bn=self.use_bn_det)]
        ...
        self.trasitionPR = nn.Sequential(
            Conv3X3(inp_planes=self.channel_nums, out_planes=self.channel_nums // 2, act_type=self.act_type,
                    use_bn=self.use_bn_rec),
            Conv1X1(inp_planes=self.channel_nums // 2, out_planes=self.colors * downscale_size ** 2,
    ...
    def forward(self, x):
        # y = self.backbone(x) + x
        # _x = torch.cat([x, x, x, x, x, x, x, x, x], dim=1)
        dn_x = self.downscale(x)
        y = self.backbonePD(dn_x)  # + _x
        y = self.headPD(y)
...
if __name__ == "__main__":
    ...
    model_hybrid = plainHYBRID(module_nums=4, channel_nums=32, num_class=2, act_type='relu', colors=3, use_bn=True,
                               rec_mode=True).cuda()
