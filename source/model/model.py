...
from .plainNetwork import plainDP, plainRP, plainHYBRID, shuffleHYBRID
...
def get_model(cfg, device):
    if cfg.model.name == 'unet_res50':
        ...
        model = UNetWithResnet50Encoder(in_channel=cfg.train_config.colors, n_classes=cfg.train_config.nclass).to(
            device)
        ...
        model = UNetWithResnet50HybridV2(in_channel=cfg.train_config.colors, n_classes=cfg.train_config.nclass).to(
            device)
        ...
        model = plainRP(module_nums=cfg.model.num_module, channel_nums=cfg.model.channel_nums, act_type='relu',
                        colors=cfg.train_config.colors, use_bn=cfg.model.use_bn)
    elif cfg.model.name == 'plainHYBRID':
        model = plainHYBRID(module_nums=cfg.model.num_module, channel_nums=cfg.model.channel_nums,
                            act_type=cfg.model.activation, colors=cfg.train_config.colors,
                            use_bn_det=cfg.model.use_bn_det, use_bn_rec=cfg.model.use_bn_rec,
                            rec_mode=cfg.model.rec_mode)
    elif cfg.model.name == 'shuffleHYBRID':
        model = shuffleHYBRID(module_nums=cfg.model.num_module, channel_nums=cfg.model.channel_nums,
                            act_type=cfg.model.activation, colors=cfg.train_config.colors,
                            use_bn_det=cfg.model.use_bn_det, use_bn_rec=cfg.model.use_bn_rec,
                            rec_mode=cfg.model.rec_mode, downscale_size=cfg.model.downscale_size)
