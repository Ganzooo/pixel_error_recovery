...
class CustomDataSet(data.Dataset):
    def __init__(self, image_folder, gt_folder, image_org_folder, train=True, augment=True, scale=2, colors=1,
                 patch_size=96, repeat=168, store_in_ram=True, use_mask_loss=True):
        ...
        if self.store_in_ram:
            ...
            for i in range(len(_imgPath)):
                _image, _gt, _imageOrg = imageio.imread(self.imageName[i], pilmode="RGB"), imageio.imread(
                    self.gtName[i], pilmode="RGB"), imageio.imread(self.imageOrgName[i], pilmode="RGB")
                if self.colors == 1:
    ...
    def __getitem__(self, idx):
        ...
        if self.store_in_ram:
            ...
            if self.colors != 1:
                _image, _gt, _fname, _imageOrg = imageio.imread(self.imageName[idx], pilmode="RGB"), imageio.imread(
                    self.gtName[idx], pilmode="L"), self.imageName[idx], imageio.imread(self.imageOrgName[idx],
                                                                                        pilmode="RGB")
            if self.colors == 1:
                _image, _gt, _fname, _imageOrg = imageio.imread(self.imageName[idx], pilmode="RGB"), imageio.imread(
                    self.gtName[idx], pilmode="L"), self.imageName[idx], imageio.imread(self.imageOrgName[idx],
        ...
        if self.train:
            ...
            hx, hy = lx, ly
            if self.colors == 1:
                _image_patch, _gt_patch, _imageOrg = _image[ly:ly + lp, lx:lx + lp, :], _gt[hy:hy + hp,
                                                                                        hx:hx + hp], _imageOrg[
                                                                                                     ly:ly + lp,
                                                                                                     lx:lx + lp, :]
            else:
            ...
            if self.augment:
                ...
                if self.colors == 1:
                    if hflip: _image_patch, _gt_patch, _imageOrg = _image_patch[:, ::-1, :], _gt_patch[:,
                                                                                             ::-1], _imageOrg[:, ::-1,
                                                                                                    :]
                    if vflip: _image_patch, _gt_patch, _imageOrg = _image_patch[::-1, :, :], _gt_patch[::-1,
                                                                                             :], _imageOrg[::-1, :, :]
...
if __name__ == '__main__':
    ...
    argment = True
    div2k = CustomDataSet(noise, noise_pixel, train=True, augment=True, scale=3, colors=3, patch_size=96, repeat=1,
                          store_in_ram=True)
