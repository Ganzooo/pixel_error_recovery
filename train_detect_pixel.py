        ...
        for step, (data) in pbar:
            ...
            if cfg.train_config.pixel_recovery:
                ...
                for b in range(_pred.shape[0]):
                    ...
                    if cfg.train_config.save_img_rec and step < 2:
            ...
            if cfg.train_config.pixel_recovery:
                ...
                for b in range(_pred.shape[0]):
                    ...
                    for _id0, _id1 in zip(index[0].data.cpu().numpy(), index[1].data.cpu().numpy()):
                        _id0 += filter_size
                        _id1 += filter_size
                        crop_neighbor = np.array(
                            _imgPred[_id0 - filter_size:_id0 + filter_size, _id1 - filter_size:_id1 + filter_size, :])
                        _imgPred[_id0, _id1, :] = (median(np.sort(crop_neighbor[:, :, 0].ravel())),
                                                   median(np.sort(crop_neighbor[:, :, 1].ravel())),
                                                   median(np.sort(crop_neighbor[:, :, 2].ravel())))
                    ...
                    if cfg.train_config.save_img_rec:
                        # fname = str(name) + '_munster_' + str(int(_idx[b])).zfill(6) + '_000019_leftImg8bit_recover.png'
                        fname = os.path.basename(_fname[b])
                        save_img(os.path.join(dirPath, str(epoch) + '_rec', fname), _imgRec.astype(np.uint8),
                                 color_domain='rgb')

                        with open(os.path.join(dirPath, "result.csv"), "a") as file:
                            file.write("{},{},{}, {:.4f}, {:.4f}, {:.4f} \n".format(str(fname), str(acc_rec), str(psnr),
                                                                                    (_end_pixel_rec - _start_pixel_err),
                                                                                    (_end_pixel_err - _start_pixel_err),
                                                                                    (_end_pixel_rec - _start_pixel_rec)))
...
def run_training(cfg, model, optimizer, scheduler, criterion, device, num_epochs, train_loader, valid_loader,
                 run_log_wandb):
...
@hydra.main(config_path="conf", config_name="config_detect_server")
